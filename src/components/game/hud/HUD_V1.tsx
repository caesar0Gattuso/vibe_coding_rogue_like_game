...
export const HUD_V1: React.FC = () => {
  ...
  const handleExitClick = () => {
    // Only pause if not already paused by something else (like level up)
    if (!isPaused) {
        togglePause(true);
    }
  ...
  const handleConfirmExit = () => {
    // This will rely on the parent App to handle unmount/remount or navigation
    // But since we are inside HUD, we might need to trigger a reload or callback
    // For now, let's use window.reload as a brute force "Back to Menu" since App default state is Menu
  ...
  return (
    <div className="absolute inset-x-0 top-0 p-4 pt-safe pointer-events-none flex flex-col gap-3 h-full pb-safe z-10">
      ...
      <div className="flex justify-between items-start pointer-events-auto">
        ...
        {/* Exit Button - Placed strategically next to weapons/stats */}
        <button 
            onClick={handleExitClick}
            className="flex items-center justify-center w-8 h-8 bg-red-500/20 border border-red-500/50 rounded-lg backdrop-blur-md text-white/80 hover:bg-red-500/40 active:scale-95 transition-all mr-auto ml-2"
